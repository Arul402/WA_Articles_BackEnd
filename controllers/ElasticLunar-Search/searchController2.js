    ...
    try {
        ...
        katturaiDetails.forEach(doc => {
            ...
            index.addDoc({
                ...
                short_desc: doc.short_desc,
                para: paraText,
                tagnames: mergeTagNames(doc.tag_names || []),
...
const normalizedQuery = query.normalize("NFC"); // Normalizes Tamil text
console.log("🔎 Fully Processed Query:", normalizedQuery);
    ...
    .map(id => new mongoose.Types.ObjectId(id)); // ✅ Convert to ObjectId
        ...
        const katturaiDocs = await Katturai.find({
            _id: { $in: objectIds }  // Directly match `id` field
        });
        
        const katturaiDetailDocs = await katturaiDetail.find({
            _id: { $in: objectIds }  // Ensure `id` is used
        });
        ...
        await Promise.all(katturaiDetailDocs.map(async (doc) => {
            const searchScore = scores[doc._id] || 0;
        ...
        const fullResults = results.map(({ ref }) => {
            ...
            const updatedDoc = katturaiDetailDocs.find(d => String(d._id) === String(ref));
